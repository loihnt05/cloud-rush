...
describe('select available seat', function() {
  this.timeout(60000)
  let driver
  let vars
  ...
  it('select available seat', async function() {
    await loginWithGlobalAccount()
    await driver.get("http://localhost:5173/flight")
    await driver.manage().window().setRect({ width: 1037, height: 773 })
    {
    ...
    await driver.sleep(1000)
    // Click passenger count or select seat
    await driver.findElement(By.css(".w-10 > .text-sm")).click()
    await driver.sleep(500)
    // Wait for and click the book/continue button
    ...
    await driver.sleep(1000)
    // Try to select another seat (should trigger alert)
    try {
      const anotherSeatButton = await driver.wait(
        until.elementLocated(By.css(".bg-accent\\/20")),
        5000
      )
      await anotherSeatButton.click()
      await driver.sleep(500)
      // Verify alert appears
      assert(await driver.switchTo().alert().getText() == "You can only select 1 seat for 1 passenger")
    } catch (e) {
      console.log('Could not find second seat button or alert did not appear:', e.message)
    }
  })
})
