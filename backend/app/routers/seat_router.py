...
def create_seat(seat: SeatCreate, db: Session = Depends(get_db), payload: dict = Depends(verify_jwt)):
    """Create a new seat"""
    return SeatService(db).create_seat(seat)

@router.get("/all", response_model=list[SeatResponse])
def list_seats(db: Session = Depends(get_db), payload: dict = Depends(verify_jwt)):
    """Get all seats"""
...
def get_seat(seat_id: int, db: Session = Depends(get_db), payload: dict = Depends(verify_jwt)):
...
def update_seat(seat_id: int, seat_data: dict, db: Session = Depends(get_db), payload: dict = Depends(verify_jwt)):
    """Update a seat"""
    try:
...
def get_available_seats_by_airplane(airplane_id: int, db: Session = Depends(get_db), payload: dict = Depends(verify_jwt)):
...
def delete_seat(seat_id: int, db: Session = Depends(get_db), payload: dict = Depends(verify_jwt)):
    """Delete a seat"""
    try:
