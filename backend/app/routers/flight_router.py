...
def list_flights(db: Session = Depends(get_db), payload: dict = Depends(verify_jwt)):
    return FlightService(db).get_all_flights()


@router.get("/{flight_id}", response_model=FlightResponse)
def get_flight(flight_id: int, db: Session = Depends(get_db), payload: dict = Depends(verify_jwt)):
